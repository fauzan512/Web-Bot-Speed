...
import { Copy, Check, Ban, Loader2, Clock, CheckCircle2, XCircle } from "lucide-react";
  ...
  return (
    <motion.div 
      ...
      className={`
        ...
        ${entry.status === 'received' ? 'bg-primary/5 border-primary/20' : 
          entry.status === 'cancelled' ? 'bg-destructive/5 border-destructive/20 opacity-75' : 
          'bg-card border-white/5 shadow-sm'}
      `}
    >
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-zinc-400 font-mono tracking-wider">
            #{entry.orderId}
          </Badge>
          <Badge variant={entry.mode === 'mexico' ? 'default' : 'secondary'} className="uppercase text-[10px]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="text-2xl font-mono font-bold tracking-tight text-white flex items-center gap-2 group cursor-pointer" onClick={copyPhone}>
            <span className="text-zinc-500 font-normal">+{countryCode}</span>
            {localNum}
      ...
      <div className="min-h-[40px] flex items-center justify-between border-t border-white/5 pt-3">
        <AnimatePresence mode="wait">
          {entry.status === 'waiting' && (
            <motion.div key="waiting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2 text-sm text-zinc-400">
          ...
          {entry.status === 'received' && (
            <motion.div key="received" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3">
              ...
              >
                {entry.otp}
                {copiedOtp ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4 opacity-50" />}
              </div>
            </motion.div>
          )}

          {entry.status === 'cancelled' && (
            <motion.div key="cancelled" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 text-sm text-destructive">
        ...
        {entry.status === 'waiting' && (
          ...
          >
            {entry.cancelLoading ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <Ban className="w-3 h-3 mr-1" />}
