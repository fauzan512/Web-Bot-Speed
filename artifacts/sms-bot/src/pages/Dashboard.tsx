...
import { LogOut, Play, Square, RefreshCcw, Terminal, Phone, Activity, Settings2, ShieldAlert } from "lucide-react";
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-background/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-lg hidden sm:block tracking-tight">HERO<span className="text-primary">BOT</span></h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Button variant="ghost" size="icon" onClick={onLogout} className="rounded-full text-zinc-400 hover:text-white hover:bg-white/5">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-6 flex flex-col h-[calc(100vh-8rem)]">
          ...
          <Card className="shrink-0">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 flex justify-between items-center bg-black/40 border border-white/5 rounded-lg p-3">
                    ...
                    <span className="text-sm font-mono font-bold text-white">
                      ...
                      <span className="text-xs text-zinc-500 ml-1 font-normal">({phData?.count || 0} stock)</span>
                    </span>
                  </div>
                </div>
              )}
              ...
              <div className="pt-2">
                {!bot.isRunning ? (
                  <Button onClick={bot.handleStart} className="w-full h-12 text-base font-bold shadow-primary/20 shadow-lg group">
                    <Play className="w-4 h-4 mr-2 fill-current group-hover:scale-125 transition-transform" />
                    START BOT
                  </Button>
                ) : (
                  <Button onClick={bot.handleStop} variant="destructive" className="w-full h-12 text-base font-bold shadow-destructive/20 shadow-lg group animate-pulse">
          ...
          <Card className="flex-1 flex flex-col min-h-0">
            <CardHeader className="py-3 px-4 border-b border-white/5 flex flex-row items-center justify-between">
              ...
              <button onClick={bot.clearLogs} className="text-xs text-zinc-500 hover:text-white transition-colors">Clear</button>
            </CardHeader>
            <CardContent className="flex-1 overflow-y-auto p-0 bg-black/50 font-mono text-[11px] leading-relaxed">
              <div className="p-4 space-y-1.5 min-h-full">
                {bot.logs.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-zinc-600 italic">Waiting for events...</div>
                ) : (
                  bot.logs.map((log) => {
                    const colors = {
                      ...
                      stop: "text-zinc-500",
                      otp: "text-emerald-300 font-bold bg-emerald-500/10 inline-block px-1 rounded",
                    };
                    return (
                      <div key={log.id} className="flex gap-3 hover:bg-white/5 px-1 -mx-1 rounded transition-colors break-words">
                        <span className="text-zinc-600 shrink-0">{format(log.timestamp, "HH:mm:ss")}</span>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-4 text-sm bg-black/40 border border-white/5 rounded-full px-4 py-1.5 shadow-inner">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {bot.purchased.length === 0 ? (
              <div className="col-span-full py-20 text-center border border-dashed border-white/10 rounded-2xl bg-black/20">
